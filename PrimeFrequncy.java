...
class Solution {
    public boolean checkPrimeFrequency(int[] nums) {
        ...
        for (int iterable_element : nums) {
            int val = map.get(iterable_element);
            boolean res = cheakPrime(val);
            if (res)
                return true;
...
public static boolean cheakPrime(int val){
   ...
   public int printLastCOunt(String s){
    s.trim();
  String [] arr =  s.split(" ");
 ...
 * 10 5 The ways are [2, 2, 2, 2, 2], [2, 2, 2, 4], [2, 4, 4], [2, 2, 6], and
 * [4, 6].
 * Example 2:
